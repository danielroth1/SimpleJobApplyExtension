...
if (typeof chrome !== 'undefined' && chrome.sidePanel && chrome.action) {
  let isOpenPerWindow = new Map()

  chrome.runtime.onInstalled.addListener(() => {
    console.log('[Background] Extension installed')
    try { chrome.sidePanel.setOptions({ path: 'index.html' }) } catch {}
    // On supported Chrome versions, make action click open the side panel automatically.
  ...
  chrome.action.onClicked.addListener(async (tab) => {
    ...
    const currentlyOpen = !!isOpenPerWindow.get(windowId)
    try {
      if (currentlyOpen) {
        // There is no direct API to close sidePanel yet; we simulate by opening a blank panel or focusing away.
        // For now we toggle state and rely on user closing manually if needed.
        isOpenPerWindow.set(windowId, false)
        console.log('[Background] Side panel toggle OFF (no close API)')
      } else {
        await chrome.sidePanel.open({ windowId })
        isOpenPerWindow.set(windowId, true)
        console.log('[Background] Side panel opened')
      }
    } catch (e) {
      console.error('[Background] Failed toggling side panel:', e)
    }
  })
}
